...
enum RPCErrorCode
{
    ...
    //! P2P client errors
    RPC_CLIENT_NOT_CONNECTED        = -9,  //! Gcoin is not connected
    RPC_CLIENT_IN_INITIAL_DOWNLOAD  = -10, //! Still downloading initial blocks
    RPC_CLIENT_NODE_ALREADY_ADDED   = -23, //! Node is already added
    RPC_CLIENT_NODE_NOT_ADDED       = -24, //! Node has not been added before
    RPC_CLIENT_NODE_NOT_BANNED      = -25, //! Node not exist in banlist
    ...
    RPC_MATCH_FAIL                  = -40, // match order fail
};
...
public:
    ...
    bool connect(const std::string& server, const std::string& port)
    {
        ...
        try {
            ...
            ip::tcp::resolver::query query(server.c_str(), port.c_str(), ip::resolver_query_base::flags());
            endpoint_iterator = resolver.resolve(query);
        }
#endif
